...
--HQ USER
        ...
        ELSE  
        V_SQL_CNDTN :=V_SQL_CNDTN|| 'AND HRD.LINE ='||''''||L_V_FSC_CODE||''''||' AND FOR_FSC ='||''''||L_V_FSC_CODE||'''';
        END IF; 
      ...
      --     V_SQL_CNDTN := V_SQL_CNDTN || ' AND  HRD.SOC_COC = ''B'''  ;     ---- BOTH
       ...
       IF(P_I_V_ETA_DATE IS NOT NULL)
        THEN                  
         V_SQL_CNDTN := V_SQL_CNDTN || 'AND TO_DATE(' || '''' || P_I_V_ETA_DATE || '''' ||', ''YYYYMMDD'') = to_date(HRD.FINAL_DEST_ETA, ''YYYYMMDD'') ';   ---- ETA DATE
       END IF;

       IF(P_I_V_ETA_PORT_DATE IS NOT NULL) 
        THEN                   
         V_SQL_CNDTN := V_SQL_CNDTN || 'AND TO_DATE(' || '''' || P_I_V_ETA_PORT_DATE || '''' ||', ''YYYYMMDD'') = to_date(HRD.POD_ETA, ''YYYYMMDD'') ';   ---- ETA PORT DATE
       END IF;
