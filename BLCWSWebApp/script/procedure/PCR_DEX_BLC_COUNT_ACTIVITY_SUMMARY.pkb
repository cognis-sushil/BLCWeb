...
-----------------------------------------------------------------------------------------------------------*/
    ...
    ELSIF( P_I_V_AGENT IS NOT NULL AND P_I_V_AGENT <>'***') THEN  
     ...
     IF( P_I_V_FSC_CDOE IS NOT NULL AND P_I_V_FSC_CDOE ='1') THEN   
         V_SQL_CNDTN := V_SQL_CNDTN || ' AND ( hrd.LINE  = ' || ''''||P_I_V_LINE||''' AND TRADE = '''||P_I_V_TRADE||''' AND AGENT  = '''||P_I_V_AGENT||''') '  ;     ---- Line
       ELSE
...
PROCEDURE PCR_DEX_BLC_COUNT_ACTIVITY_SUMMARY_SEARCH_DETAILE ( 
  ...
  v_sql_1 := 'SELECT DISTINCT hrd.PK_BL_NO, hrd.BL_TYPE, hrd.DEX_STATUS, hrd.BL_PRINT_STATUS,  CASE WHEN (rev.FK_INVOICE_NO IS NOT NULL AND  rev.FLAG_PREPAID_COLLECT = ''P'' ) THEN ''Y'' ELSE ''u'' END AS Generated_Invoices  , CASE WHEN (rev.INVOICE_PRINTED_FLAG  IS NOT NULL AND rev.FLAG_PREPAID_COLLECT = ''P'' ) THEN ''Y'' ELSE ''u'' END AS Printed_Invoices ,  hrd.BL_RELEASED_FLAG,  hrd.SPECIAL_RELEASE_REQUESTED FROM rcltbls.dex_bl_header  hrd LEFT OUTER JOIN  DEX_BL_REVENUE rev ON hrd.PK_BL_NO = rev.FK_BL_NO  WHERE 1=1 ';
